...
def render_ranking(df_enc: pd.DataFrame) -> None:
    st.markdown('<p class="section-title">🏆 Ranking de preguntas Likert</p>', unsafe_allow_html=True)
    ...
    fig = go.Figure(go.Bar(
        x=df_prom["Promedio"], y=df_prom["Pregunta"], orientation="h",
        text=[f"  {p:.2f}" for p in df_prom["Promedio"]],
        textposition="outside", textfont=dict(color="#333333", size=12),
        marker_color=colors, marker_line=dict(width=1, color="#FFFFFF"),
        cliponaxis=False,
    ))
    fig.update_layout(
        height=500, margin=dict(t=10, b=30, l=10, r=80),
        plot_bgcolor="#FFFFFF", paper_bgcolor="#FFFFFF",
        xaxis=dict(range=[0, 5.5], tickfont=dict(color="#888", size=10),
                   gridcolor="#ECECEC", zeroline=False, title="Promedio (1-5)",
    ...
    with st.expander("📋 Tabla de promedios", expanded=False):
        st.dataframe(df_prom[["Pregunta", "Promedio", "Respuestas"]].sort_values("Promedio", ascending=False),
